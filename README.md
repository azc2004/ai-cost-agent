...
| `services.py` | 5개 서비스 호출 구조(**데이터 딕셔너리**) + 비용 계산 함수. 데이터만 고치면 단계/횟수/토큰 변경 |
...
- 차트는 dataviz 가이드라인의 검증 팔레트(라이트/다크)를 사용한다. 5개 서비스는 고정 순서(슬롯 1~5)의 색상을 쓰며, 라이트 모드에서 일부 계열은 대비가 낮아 **정확한 값은 통합 비교표를 기준**으로 확인한다.
